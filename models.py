...
class Repo(db.Model):
    ...
    def __init__(self, newid,  name,url):
        self.RepoId = newid
        self.RepoName = name
        self.RepoUrl = url
...
class BugProbability(db.Model):
    __tablename__ = 'BugProbabilities'
...
class RepoStructure(db.Model):
    __tablename__ = 'RepoStructures'

    RepoId = db.Column(db.BigInteger, db.ForeignKey('Repos.RepoId'), primary_key=True)
    RepoName = db.Column(db.Text, nullable = False)
    RepoUrl = db.Column(db.Text, nullable = False)
    RepoTree = db.Column(JSON, nullable = False)

    def __init__(self, newid, name,url, repotree):
        self.RepoId = newid
        self.RepoName = name
        self.RepoUrl = url
